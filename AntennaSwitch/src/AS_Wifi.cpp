...
void NAMEchange()
{
  ...
  tempStr+="background-image: url('bg.jpg')";
  tempStr+="<br><br><br><br><br><br>";
 tempStr += "<label for=\"ant1\" style=\"margin-left: 500px;\">ANTENA 1: </label><input type=\"text\" id=\"ant1\" value=\" " + antName[0] + "\" name=\"ant1\"><br><br>";
  tempStr += "<label for=\"ant2\">ANTENA 2: </label><input type=\"text\" id=\"ant2\" value=\" " + antName[1] + "\" name=\"ant2\"><br><br>";
  tempStr += "<label for=\"ant3\">ANTENA 3: </label><input type=\"text\" id=\"ant3\" value=\" " + antName[2] + "\"  name=\"ant3\"><br><br>";
  tempStr += "<label for=\"ant4\">ANTENA 4: </label><input type=\"text\" id=\"ant4\" value=\" " + antName[3] + "\"  name=\"ant4\"><br><br>";
  tempStr += "<label for=\"ant5\">ANTENA 5: </label><input type=\"text\" id=\"ant5\" value=\" " + antName[4] + "\"  name=\"ant5\"><br><br>";
...
void setup(void) {
 // uint32_t brown_reg_temp = READ_PERI_REG(RTC_CNTL_BROWN_OUT_REG); //save WatchDog register
  ...
  else
  {
    ...
    strcpy(antennaName[0], "ANTENA 1");    // null out names to start
    strcpy(antennaName[1], "ANTENA 2");    // null out names to start
    strcpy(antennaName[2], "ANTENA 3");    // null out names to start
    strcpy(antennaName[3], "ANTENA 4");    // null out names to start
    strcpy(antennaName[4], "ANTENA 5");    // null out names to start
    strcpy(antennaName[5], "ANTENA 6");    // null out names to start
  ...
  while(WiFi.status() != WL_CONNECTED)
  {
   ...
   if (!SPIFFS.begin(true)) {
        DEBUG_PORT.println("SPIFFS Mount Failed");
          Serial.println("SPIFFS Mount Failed");
        return;
    }
    else {
      Serial.println("SPIFF MOUNTED SUCESSFULL");
    }
        // HOST_PORT.print("IP:");             // Notify host
