...
int  WIND_THRESHOLD = 25; // Maximum windspeed for green, otherwise the LED turns yellow
int LOOP_INTERVAL = 5000; // ms - interval between brightness updates and lightning strikes
...
int BRIGHTNESS= 20; // 20-30 recommended. If using a light sensor, this is the initial brightness on boot.
...
EEPROM.get(0, WIND_THRESHOLD); // Retrieve th from EEPROM address 0
EEPROM.get(sizeof(int), LOOP_INTERVAL); // Retrieve loop from EEPROM address sizeof(int)
EEPROM.get(2 * sizeof(int), BRIGHTNESS); // Retrieve brightness from EEPROM address 2 * sizeof(int)
 ...
 // wfm.setDebugOutput(false);
  ...
  WiFiManagerParameter custom_text_box("mytext","","Enter Threshold",30);
   WiFiManagerParameter custom_text_box1("mytext","","Enter loop value",30);
      WiFiManagerParameter custom_text_box2("mytext","","Enter Brightness",30);
  wfm.addParameter(&custom_text_box);
  wfm.addParameter(&custom_text_box1);
  wfm.addParameter(&custom_text_box2);

  if(!wfm.autoConnect("ESP32AP","12345678")){
...
int th=(int)custom_text_box.getValue();
int loop=(int)custom_text_box1.getValue();
int brightness=(int)custom_text_box2.getValue();
Serial.println("THOLD : "+(String)th);
Serial.println("LOOP : "+(String)loop);
Serial.println("Brightness : "+(String)brightness);
EEPROM.put(0, th); // Store th at EEPROM address 0
EEPROM.put(sizeof(int), loop); // Store loop at EEPROM address sizeof(int)
EEPROM.put(2 * sizeof(int), brightness); // Store brightness at EEPROM address 2 * sizeof(int)
EEPROM.commit(); // Save changes to EEPROM

Serial.println("------SAVED SUCESSFULLY--------");
